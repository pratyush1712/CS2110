...
public class GameComponent extends JPanel implements MouseListener {
    ...
    private void timeout() {
        ...
        if (isActive && targetCount == maxTargets) {
            stopGame();
        } else if (isActive) {
            target.respawn(getWidth() - getTargetRadius(), getHeight() - getTargetRadius());
            targetCount++ ;
            repaint();
        }
    }
    ...
    public void mousePressed(MouseEvent e) {
        ...
        if (isActive && target.checkHit(e.getPoint().x, e.getPoint().y)) {
            setScore(getScore()+1);
            repaint();
        }
    }
    ...
    private static class Target {
        ...
        boolean checkHit(int cx, int cy) {
            ...
            if (!isHit && (Math.pow(cx-x, 2) + Math.pow(cy-y, 2)) <= Math.pow(radius, 2)) {
                isHit=true;
                return true;
            }
            return false;
